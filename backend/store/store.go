...
func NewMemoryStore() *MemoryStore { //constructor function
	return &MemoryStore{
		menus: []Menu{
...
func (s *MemoryStore) List(menuType string) []Menu { //List
	s.mu.RLock()
	defer s.mu.RUnlock()
	if menuType == "" {
...
func (s *MemoryStore) Get(id int) (Menu, error) { //Get
	s.mu.RLock()
	defer s.mu.RUnlock()
	for _, m := range s.menus {
		if m.ID == id {
...
func (s *MemoryStore) Add(m Menu) Menu { //Add
	s.mu.Lock()
	defer s.mu.Unlock() //lock the mutex to ensure thread safety
	m.ID = s.nextID
	s.nextID++
...
func (s *MemoryStore) Delete(id int) error { //Delete
	s.mu.Lock()
	defer s.mu.Unlock() //lock the mutex to ensure thread safety
